...
VALID_SOURCE_CATEGORIES = {"mainstream", "politics", "magazine", "international", "institutional", "party"}
...
def _save_sentiment(payload: dict[str, Any]) -> None:
    ...
    SENTIMENT_FILE.write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
...
def _append_pipeline_error(*, message: str, article_id: str | None, provider: str | None) -> None:
...
def _label_map_to_scores(labels: dict[str, str]) -> dict[str, float]:
    return {candidate: LABEL_TO_SCORE[label] for candidate, label in labels.items() if label in LABEL_TO_SCORE}
...
def _build_sentiment_prompt(article: dict[str, Any]) -> tuple[str, str]:
    ...
    candidate_list = ", ".join(candidate_hint) if isinstance(candidate_hint, list) else ""
...
def _ensure_article_defaults(article: dict[str, Any]) -> None:
    ...
    article["_sentiment_scores"] = _normalize_score_map(article.get("_sentiment_scores"))
    if isinstance(article.get("sentiment_score"), (int, float)):
        article["sentiment_score"] = round(_clamp_score(float(article["sentiment_score"])), 4)
    else:
...
def _aggregate_average(accumulator: dict[str, dict[str, list[float]]]) -> dict[str, dict[str, float]]:
    aggregated: dict[str, dict[str, float]] = {}
    for outer_key in sorted(accumulator):
        inner = accumulator[outer_key]
        aggregated[outer_key] = {inner_key: _average(scores) for inner_key, scores in sorted(inner.items())}
    return aggregated
...
def _validate_sentiment_payload(payload: dict[str, Any]) -> None:
    ...
    if errors:
        ...
        raise ValueError(f"Sentiment payload schema validation failed at {where}: {first.message}")
...
def analyze_sentiment() -> dict[str, Any]:
    ...
    for article in articles:
        ...
        label_map = _normalize_sentiment_labels(article.get("sentiment_per_candidate"))
        score_map = _normalize_score_map(article.get("_sentiment_scores"))
        if not score_map:
            score_map = _label_map_to_scores(label_map)
            if score_map:
                article["_sentiment_scores"] = score_map

        if not score_map:
            system, user = _build_sentiment_prompt(article)
            provider_name: str | None = None
            try:
                response = ai_client.call_with_fallback(system=system, user=user, max_tokens=350)
                provider_name = response.get("provider") if isinstance(response.get("provider"), str) else None
                score_map = _extract_scores_from_ai_response(str(response.get("content", "")))
            except Exception as exc:
                _append_pipeline_error(message=str(exc), article_id=article_id, provider=provider_name)
                logger.warning("Sentiment extraction failed for article %s: %s", article_id or "<missing-id>", exc)
                continue

            label_map = {candidate: _score_to_label(score) for candidate, score in score_map.items()}
        ...
        normalized_topics = [topic for topic in topics if isinstance(topic, str) and topic in VALID_TOPICS]
        if not normalized_topics:
            normalized_topics = ["eleicoes"]

        source_category = article.get("source_category")
        normalized_source = source_category if isinstance(source_category, str) and source_category in VALID_SOURCE_CATEGORIES else "mainstream"
    ...
    unique_topics = {topic for candidate_topics in by_topic.values() for topic in candidate_topics}

    payload: dict[str, Any] = {
        "updated_at": _compute_updated_at(articles),
        "article_count": processed_articles,
